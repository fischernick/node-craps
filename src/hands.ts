...
const startingBalance: number =
  config.startingBalance ?? defaultConfig.startingBalance;
...
console.log(
  `Show detail: ${showDetail} // displayTables: ${config.displayTables}`,
);
    ...
    if (
      ...
      roll = () => {
        if (shootCount >= parsedRolls.length) {
          console.log(
            `[roller] shootCount: ${shootCount} >= rolls.length: ${parsedRolls.length}`,
          );
          return rollD6();
        }
      ...
      console.log(
        `[rolls] loaded ${parsedRolls.length} rolls from ${config.handsFile}`,
      );
      console.log(`[rolls] ${JSON.stringify(parsedRolls)}`);
    } else {
      throw new Error(
        "Invalid roll format - each roll must have d1 and d2 numbers",
      );
    }
  } catch (err) {
...
if (!sessionSummary) {
  throw "missin summary";
}
if (!sessionSummary.dist) {
  throw "missin summary";
}
...
for (const [key, value] of sessionSummary.dist.entries()) {
  ...
  console.log(
    `│ ${keyStr} | ${countStr} │ ${expectedStr} │ ${diffStr} │ ${diffPctStr} │`,
  );
}
console.log(`└─────┴───────┴──────────┴────────┴────────┘`);
...
const balanceStr =
  sessionSummary.balance > startingBalance
    ? chalk.green(psCt(sessionSummary.balance, 7))
    : chalk.red(psCt(sessionSummary.balance, 7));
...
console.log(
  `│ Net Come Out Wins:│ ${psCt(sessionSummary.netComeOutWins, 8)} │`,
);
console.log(`│ Neutrals:         │ ${psCt(sessionSummary.neutrals, 8)} │`);
console.log(`│ Hand Count:       │ ${psCt(sessionSummary.handCount, 8)} │`);
console.log(`└───────────────────┴──────────┘`);

export const buildHandsSummary = (
  hands: { history: Result[]; balance: number; summary: Summary }[],
  handCount: number,
  handsPerRow: number,
): string[] => {
  ...
  for (let i = 0; i < handsPerRow && currentHandCount < hands.length; i++) {
    const h = hands[currentHandCount++];
    delete h.summary.dist;
    let sfx = (n: number): string => {
      return "";
    };
    ...
    handsSummary[2] += " │" + pd("$" + h.summary.balance.toString()) + sfx(2);
    handsSummary[3] += " │" + pd(h.summary.rollCount.toString()) + sfx(3);
    handsSummary[4] += " │" + pd(h.summary.pointsSet.toString()) + sfx(4);
    handsSummary[5] += " │" + pd(h.summary.pointsWon.toString()) + sfx(5);
    handsSummary[6] += " │" + pd(h.summary.comeOutWins.toString()) + sfx(6);
    handsSummary[7] += " │" + pd(h.summary.comeOutLosses.toString()) + sfx(7);
    handsSummary[8] += " │" + pd(h.summary.netComeOutWins.toString()) + sfx(8);
    handsSummary[9] += " │" + pd(h.summary.neutrals.toString()) + sfx(9);
...
//   hands.forEach((h, handIndex) => {
//     console.log(`\nHand ${handIndex + 1} History:`);
//     h.history.forEach((roll: Result, rollIndex: number) => {
