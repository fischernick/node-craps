...
export function getBettingStrategy(
  strategyName: BettingStrategy,
): BettingStrategyFunc {
  switch (strategyName) {
...
export function minPassLineOnly(opts: HandOptions): BetDictionary {
  ...
  const bets = existingBets ?? new BetDictionary();

  if (process.env.DEBUG) {
    const shouldMakeNewPassLineBet =
      hand.isComeOut && !bets.getBet(BetPoint.Pass);
    console.log(
      `[MPLO decision] Make new pass line bet? isComeOut=${hand.isComeOut}, noExistingBet=${!bets.getBet(BetPoint.Pass)} => ${shouldMakeNewPassLineBet}`,
    );
  }
  ...
  if ((hand?.isComeOut ?? false) && bets.getBet(BetPoint.Pass) === undefined) {
    ...
    if (process.env.DEBUG)
      console.log(`[MPLO did] bet: ${JSON.stringify(bets)}`);
  }
...
export function minPassLineMaxOdds(opts: HandOptions): BetDictionary {
  ...
  if (process.env.DEBUG)
    console.log(
      `[MPLMO decision] make a new pass odds bet?: ${!hand.isComeOut} && ${!bets.getBet(BetPoint.PassOdds)} => ${!hand.isComeOut && !bets.getBet(BetPoint.PassOdds)}`,
    );
  ...
  if ((hand?.isComeOut ?? false) === false && !bets.getBet(BetPoint.PassOdds)) {
    ...
    const oddsAmount =
      rules.maxOddsMultiple[hand.point] *
      (bets.getBet(BetPoint.Pass)?.amount ?? 0);
...
export function dontComeWithPlaceBets(opts: HandOptions): BetDictionary {
  ...
  const dontComePointedBet: boolean = !!DontComeBetPoints.find((bet) =>
    theseBets?.getBet(bet),
  );
  ...
  if (dontComePointedBet || dontComeBet) {
    ...
    if (
      !bets?.getBet(BetPoint.DontComePoint5) &&
      !bets?.getBet(BetPoint.Place5)
    )
      theseBets.addBet(BetPoint.Place5, 15);
    if (
      !bets?.getBet(BetPoint.DontComePoint6) &&
      !bets?.getBet(BetPoint.Place6)
    )
      theseBets.addBet(BetPoint.Place6, 18);
    if (
      !bets?.getBet(BetPoint.DontComePoint8) &&
      !bets?.getBet(BetPoint.Place8)
    )
      theseBets.addBet(BetPoint.Place8, 18);
    if (
      !bets?.getBet(BetPoint.DontComePoint9) &&
      !bets?.getBet(BetPoint.Place9)
    )
