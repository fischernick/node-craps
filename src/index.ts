...
export function shoot(before: Result, dice: DieResult[]): Result {
  ...
  const after: Result = {
    die1: sortedDice[0],
    die2: sortedDice[1],
    diceSum: dice.reduce((m: number, r: number) => {
      return m + r;
    }, 0),
    result: undefined,
    isComeOut: undefined,
...
export function playHand(
  ...
  while (hand.result !== HandResult.SEVEN_OUT) {
    ...
    if (process.env.DEBUG && bets.newBetSum)
      console.log(`[bet] new bet $${bets.newBetSum} ($${balance})`);
    bets.newBetSum = 0;

    if (opts.displayTables) displayTable(true, bets, hand.point, balance, hand);

    hand = shoot(hand, [roll(), roll()]);

    if (process.env.DEBUG)
      console.log(`[roll] ${hand.result} (${hand.diceSum})`);

    let newPayouts: Payout[] | undefined = [];
