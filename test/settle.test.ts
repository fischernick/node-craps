...
test("passLine", (suite) => {
  suite.test("comeout win", function (t) {
    ...
    t.notOk(
      result.bets.getBet(BetPoint.Pass),
      "pass line bet is cleared upon comeout win",
    );
  ...
  suite.test("comeout loss", function (t) {
    ...
    t.notOk(
      result.bets.getBet(BetPoint.Pass),
      "pass line bet is cleared upon comeout loss",
    );
  ...
  suite.test("point win", function (t) {
    ...
    t.notOk(
      result.bets.getBet(BetPoint.Pass),
      "pass line bet is cleared upon point win",
    );
  ...
  suite.test("point loss", function (t) {
    ...
    t.notOk(
      result.bets.getBet(BetPoint.Pass),
      "pass line bet is cleared upon seven out",
    );
...
test("passOdds", (suite) => {
  suite.test("odds bet, no win", function (t) {
    ...
    t.strictSame(
      result.bets,
      passLineWithOddsBet,
      "settled bets are same as initial bets",
    );
    t.end();
  });
